...
namespace stdext
{
	...
	std::string strformat(const char* fmt, Types... args)
	{
		char buffer[sz];
		sprintf_s(buffer, sz, fmt, args...);
		return buffer;
	}
}
...
public:
	...
	static void ReadARGB(fs::path file_path, std::vector<uint8_t>& data)
	{
		uint32_t width;
		uint32_t height;
		png_byte color_type;
		png_byte bit_depth;
		return ReadARGB(file_path, width, height, color_type, bit_depth, data);
	}

	static void ReadARGB(fs::path file_path, uint32_t& width, uint32_t& height, std::vector<uint8_t>& data)
	{
		png_byte color_type;
		png_byte bit_depth;
		return ReadARGB(file_path, width, height, color_type, bit_depth, data);
	...
	static void ReadARGB(fs::path file_path, uint32_t& width, uint32_t& height, png_byte& color_type, png_byte& bit_depth, std::vector<uint8_t>& data)
	{
		char header[8];
		...
		for (uint32_t y = 0; y < height; y++) {
			...
			if (color_type == PNG_COLOR_TYPE_GRAY) {
				for (uint32_t i = 0; i < width; i++) {
					auto dst_offset = (y * width + i) * 4;
					for (int c = 0; c < 3; c++)
						data[dst_offset + c] = row_bytes[i];
					data[dst_offset + 3] = 0xFF;
				}
			}
			else if (color_type == PNG_COLOR_TYPE_RGB) {
				for (uint32_t i = 0; i < width * 3; i += 3) {
					auto dst_offset = (y * width + i / 3) * 4;
					for (int c = 0; c < 3; c++)
						data[dst_offset + c] = row_bytes[i + c];
					data[dst_offset + 3] = 0xFF;
				}
			}
			else if (color_type == PNG_COLOR_TYPE_RGB_ALPHA) {
				for (uint32_t i = 0; i < width * 4; i += 4) {
					auto dst_offset = (y * width + i / 4) * 4;
					for (int c = 0; c < 4; c++)
						data[dst_offset + c] = row_bytes[i + c];
				}
			}
...
class PngWriter {
public:
	static void WriteGray(std::vector<uint8_t>& buffer, uint32_t width, uint32_t height, fs::path file_path)
	{
		std::vector<png_bytep> png_rows;
		png_rows.resize(height);
		for (int i = 0; i < height; i++) {
			png_rows[i] = &buffer[i * width];
		}
	...
	static void WriteARGB(std::vector<uint8_t>& buffer, uint32_t width, uint32_t height, fs::path file_path)
	{
		std::vector<png_bytep> png_rows;
		png_rows.resize(height);
		for (int i = 0; i < height; i++) {
			png_rows[i] = &buffer[i * width * 4];
		}
